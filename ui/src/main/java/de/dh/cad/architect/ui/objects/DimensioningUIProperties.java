...
public class DimensioningUIProperties extends BaseObjectUIRepresentation {
    public static final String KEY_PROPERTY_LENGTH = "length";

    public DimensioningUIProperties() {
    ...
    protected void addProperties(Map<String, Collection<UiProperty<?>>> result, BaseObject bo, UiController uiController) {
        ...
        Collection<UiProperty<?>> properties = result.computeIfAbsent("Bemaßung", cat -> new ArrayList<>());
        properties.add(
                new UiProperty<Length>(bo, KEY_PROPERTY_LENGTH, "Länge", PropertyType.Length, anchor2.isHandle()) {
